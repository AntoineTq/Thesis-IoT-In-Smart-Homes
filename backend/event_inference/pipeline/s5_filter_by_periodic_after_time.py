...
def eval_individual_device(dataset, dname, random_state, specified_models=None):
    ...
    # exit(1)
    for i in range(len(test_feature)):
        if test_hosts[i] in mac_dic or test_hosts[i] in local_mac_list or test_hosts[i]=='multicast': #or ':' in test_hosts[i]: #had to remove it to allow traffic to my router
