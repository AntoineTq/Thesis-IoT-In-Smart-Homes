...
def extract_pcap(in_pcap, out_txt, dev_name, ip_host):
    ...
    for packet in filter(None, out.decode('utf-8').split('\n')):
        ...
        if len(packet) < 13 or len(packet[6]) >= 18 or packet[4] == 'ICMP' or packet[4] == 'CLASSIC-STUN':
            continue
        ...
        if dev_name == 'govee-led1' or dev_name == 'philips-bulb':
            pass
        elif cur_time.astype(float) < 1630688400:  # sep3 1pm sep8 13pm: 1631120400
            continue
        """elif cur_time.astype(float) > 1631120400:
            continue"""
    ...
    print('Results:', in_pcap, result.shape)
...
def run(files, out_dir, ip_hosts):
    ...
    for f in files:
        ...
        if os.path.isfile(out_txt):  # os.path.isfile(out_txt):
            ...
            ip_host = ip_hosts.get(dev_name, None)
            if ip_host is None:
                continue
            count_tmp = extract_pcap(f, out_txt, dev_name, ip_host)
            if count_tmp != 1:
...
def main():
    ...
    in_txt = sys.argv[1]
    out_dir = sys.argv[2]
    str_num_proc = sys.argv[3] if len(sys.argv) == 4 else "4"
    ...
    ip_hosts_all = {}
    model_file = os.path.join(event_inference_dir, "ip_host", "ip_host_idle.model")
