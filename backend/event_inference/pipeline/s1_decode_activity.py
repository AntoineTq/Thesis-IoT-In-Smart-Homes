...
def main():
    ...
    ip_hosts_all = {}
    if in_txt.endswith('routine-dataset.txt') or in_txt.endswith('predict.txt'):
        model_file = os.path.join(event_inference_dir, "ip_host", "routines.model")
    else:
        model_file = os.path.join(event_inference_dir, "ip_host", "activity.model")
